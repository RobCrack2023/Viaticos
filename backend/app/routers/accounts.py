...
def list_fotos_movement(mv_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    account = _get_account(current_user, db)
    mv = db.query(AccountMovement).filter(AccountMovement.id == mv_id, AccountMovement.account_id == account.id).first()
    if not mv:
        raise HTTPException(status_code=404, detail="Movimiento no encontrado")
    photos = db.query(MovementPhoto).filter(MovementPhoto.movement_type=="cc", MovementPhoto.movement_id==mv_id).all()
    all_fotos = []
...
async def add_foto_movement(mv_id: int, foto: UploadFile = File(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    account = _get_account(current_user, db)
    mv = db.query(AccountMovement).filter(AccountMovement.id == mv_id, AccountMovement.account_id == account.id).first()
    if not mv:
        raise HTTPException(status_code=404, detail="Movimiento no encontrado")
    ext = os.path.splitext(foto.filename)[1] or ".jpg"
    filename = f"acc_{mv_id}_x{uuid.uuid4().hex[:8]}{ext}"
    path = os.path.join(settings.UPLOADS_DIR, filename)
    os.makedirs(settings.UPLOADS_DIR, exist_ok=True)
    async with aiofiles.open(path, "wb") as f:
        await f.write(await foto.read())
    photo = MovementPhoto(movement_type="cc", movement_id=mv_id, foto_path=filename)
    db.add(photo)
    db.commit()
...
def delete_foto_movement(mv_id: int, foto_id: int, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    account = _get_account(current_user, db)
    db.query(AccountMovement).filter(AccountMovement.id==mv_id, AccountMovement.account_id==account.id).first() or \
        (_ for _ in ()).throw(HTTPException(404, "Movimiento no encontrado"))
    photo = db.query(MovementPhoto).filter(MovementPhoto.id==foto_id, MovementPhoto.movement_type=="cc").first()
